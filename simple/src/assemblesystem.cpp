 ...
 * LastEditTime: 2026-02-10 02:02:39
 * FilePath: \simple\src\assemblesystem.cpp
 * Description: 
 * 
...
void AssembleSystem::MomentumCoefs(StructureMesh &mesh, SolverSettings &solversettings, MaterialSettings &material)
{
    ...
    //黏度场
    const auto& mu = material.ViscosityExport();
    float mul, mur;
    ...
    float aE, aW, aN, aS, aT, aB, aP, aP_0;
    for(int k=0; k < cellnum[2]; ++k)
    {
        for(int j=0; j < cellnum[1]; ++j)
        {
            for(int i=0; i < cellnum[0]; ++i)
            {
                ...
                ul = uf[i+1][j][k];
                ur = uf[i+1][j][k];
                aE = inport_kernel.CalCoefA(conv_scheme, area_vol[0], idx, ul, ur, mul, mur, rho, -1.0f); 
                // std::cout << "aE: " << aE << "\n"; 
                ...
                if(i == 0)
                {
                    ...
                    rho = 0.5f * (density[i-1][j][k] + density[i][j][k]);
                    mul = mu[i-1][j][k];
                    mur = mu[i][j][k];
                }
                ...
                ul = uf[i][j][k];
                ur = uf[i][j][k];
                aW = inport_kernel.CalCoefA(conv_scheme, area_vol[0], idx, ul, ur, mul, mur, rho, 1.0f); 
                // std::cout << "aW: " << aW << "\n"; 
                ...
                vl = vf[i][j+1][k];
                vr = vf[i][j+1][k];
                aN = inport_kernel.CalCoefA(conv_scheme, area_vol[1], idy, vl, vr, mul, mur, rho, -1.0f); 
                // std::cout << "aN: " << aN << "\n";
                ...
                if(j == 0)
                {
                    ...
                    rho = 0.5f * (density[i][j-1][k] + density[i][j][k]);
                    mul = mu[i][j-1][k];
                    mur = mu[i][j][k];
                }
                ...
                vl = vf[i][j][k];
                vr = vf[i][j][k];
                aS = inport_kernel.CalCoefA(conv_scheme, area_vol[1], idy, vl, vr, mul, mur, rho, 1.0f); 
                // std::cout << "aS: " << aS << "\n";
                ...
                if(dim == 3)
                {
                    ...
                    wl = wf[i][j+1][k];
                    wr = wf[i][j+1][k];
                    aT = inport_kernel.CalCoefA(conv_scheme, area_vol[2], idz, wl, wr, mul, mur, rho, -1.0f);
                    // std::cout << "aT: " << aT << "\n";
                    ...
                    if(k == 0)
                    {
                        ...
                        rho = 0.5f * (density[i][j][k-1] + density[i][j][k]);
                        mul = mu[i][j][k-1];
                        mur = mu[i][j][k];
                    }
                    ...
                    wl = wf[i][j][k];
                    wr = wf[i][j][k];
                    aB = inport_kernel.CalCoefA(conv_scheme, area_vol[2], idz, wl, wr, mul, mur, rho, 1.0f);
                    // std::cout << "aB: " << aB << "\n";             
                }

                rho = density[i][j][k];
                aP_0 = rho * area_vol[3] * idt;
                aP = aP + aE + aW + aN + aS + aT + aB + aP_0 - sP * area_vol[3];
                std::cout << "i,j,k,aP,aE,aW,aN,aS,aP0: " << i<< " " << j << " " << k << " " << aP << " " << aE << " " << aW << " " << aN << " " << aS << " " << aP_0;
                std::cout << std::endl;
                // std::cout << "aP_0: " << aP_0 << " aP: " << aP << "\n";
            }
        }
    }
}
