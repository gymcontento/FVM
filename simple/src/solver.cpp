 ...
 * LastEditTime: 2026-01-11 00:29:30
 * FilePath: \FVM\heat_conduction\src\solver.cpp
 * Description: 
 * 
...
std::vector<float> Solver::CalScalarNorm2(StructureMesh &mesh, SolverSettings& solversettings, std::string var)
{
...
void Solver::GaussSiedelMethod(int& nlitera_nums, StructureMesh& mesh, PostProcess& postprocess, SolverSettings& solversettings)
{
    // 定义与网格相关的变量
    const auto& dim = mesh.dim_export();
    const auto& cellnum = mesh.cellnum_export();
    auto& cellcoef = mesh.cellcoeff;
    ...
    for(int i=1; i<solversettings.t_iter_nums+1; ++i)
    {
        // 保存旧温度值用于计算残差
        auto t_old = mesh.t;
        ...
        for(int k=0; k < cellnum[2]; ++k)
        {
            for(int j=0; j < cellnum[1]; ++j)
            {
                for(int i=0; i < cellnum[0]; ++i)
                {
                    ...
                    float du = solversettings.relax_coef * (t_new - t_old[i][j][k]);
        ...
        if(rel_norm < solversettings.linsol_residual || i == solversettings.t_iter_nums){
            solversettings.linsol_iters_nums += i;
            if(nlitera_nums % res_freq || nlitera_nums == 1 || nlitera_nums == nl_total_steps){
                // 输出到控制台
                std::cout << "it_nl, it, tot_it, norm2, init, max, rel " 
                        << nlitera_nums << " " 
                        << i << " " 
                        << solversettings.linsol_iters_nums << " " 
                        << norm2 << " " 
                        << initial_norm << " " 
                        << max_norm << " " 
                        << rel_norm << std::endl;
...
void Solver::JacobiIteraMethod(int& nlitera_nums, StructureMesh& mesh, PostProcess& postprocess, SolverSettings& solversettings)
{
    // 定义与网格相关的变量
    const auto& dim = mesh.dim_export();
    const auto& cellnum = mesh.cellnum_export();
        ...
        for(int k=0; k < cellnum[2]; ++k)
        {
            for(int j=0; j < cellnum[1]; ++j)
            {
                for(int i=0; i < cellnum[0]; ++i)
                {
                    ...
                    float du = solversettings.relax_coef * (t_new - t0[i][j][k]);
