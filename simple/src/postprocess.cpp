 ...
 * LastEditTime: 2026-02-04 17:41:45
 * FilePath: \simple\src\postprocess.cpp
 * Description: 
 * 
     ...
     StructureMesh& mesh, std::string filename)
{
    ...
    const std::vector<std::vector<std::vector<float>>>& t = mesh.t;
...
void PostProcess::WriteVTKCollocated_vel(
     StructureMesh& mesh, std::string filename)
{
    const std::vector<int>& cellnum = mesh.cellnum_export();
    const std::vector<int>& nodenum = mesh.nodenum_export();
    ...
    const std::vector<std::vector<std::vector<float>>>& u = mesh.u;
    const std::vector<std::vector<std::vector<float>>>& v = mesh.v;
    const std::vector<std::vector<std::vector<float>>>& w = mesh.w;
    ...
    // 假设 u, v, w 是三维数组，维度为 ncx x ncy x ncz
    std::vector<std::string> vars = {"u", "v", "w"};
    for (const auto& var : vars) {
        // 写入VTK头
        vtkFile << var << " 1 " << totalcnum << " float\n";
...
void PostProcess::WriteVTKCollocated_pre(
     StructureMesh& mesh, std::string filename)
{
    const std::vector<int>& cellnum = mesh.cellnum_export();
    const std::vector<int>& nodenum = mesh.nodenum_export();
    ...
    const std::vector<std::vector<std::vector<float>>>& p = mesh.p;
    const std::vector<std::vector<std::vector<float>>>& pp = mesh.pp;
    ...
    //定义数据块
    vtkFile << "FIELD FieldData 3\n";

    // 写入压力数据
    // ncell = ncx * ncy * ncz
    std::vector<std::string> vars = {"p", "pp"};
    for (const auto& var : vars) {
        // 写入VTK头
        vtkFile << var << " 1 " << totalcnum << " float\n";
